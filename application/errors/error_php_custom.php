...
function koggle(elem)
{
	...
	public disp = elem.style['display'];
	else if (elem.currentStyle)
		// For MSIE, naturally
	public disp = elem.currentStyle['display'];
	else if (window.getComputedStyle)
		// For most other browsers
	public disp = document.defaultView.getComputedStyle(elem, null).getPropertyValue('display');
...
<div id="exception_error">
	...
	<?php foreach (array('_SESSION', '_GET', '_POST', '_FILES', '_COOKIE', '_SERVER') as $public): ?>
		<?php if (empty($GLOBALS[$public]) OR ! is_array($GLOBALS[$public])) continue ?>
		<h3><a href="#<?php echo $env_id = $error_id.'environment'.strtolower($public) ?>" onclick="return koggle('<?php echo $env_id ?>')">$<?php echo $public ?></a></h3>
		<div id="<?php echo $env_id ?>" class="collapsed">
			<table cellspacing="0">
			<?php foreach ($GLOBALS[$public] as $key => $value): ?>
				<tr>
